...
async function main(): Promise<void> {
    ...
    const embeddedCaptcha = await capture({
        url: new URL('/embedded-captcha', fixtureUrl).toString(),
        waitForMs: 0,
        resizeWaitMs: 0,
    });
    assert.equal(embeddedCaptcha.status, 200);
    assert.equal(embeddedCaptcha.body.ok, true);
